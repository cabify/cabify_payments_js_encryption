...
namespace :build do
  ...
  task :bundle => ["build:clean"] do
    files = %W[
      ...
      #{LIB_DIR}/cabify_payments.js
      #{BUILD_DIR}/bundle_footer.js
    ]
    `cat #{files.join(' ')} >> #{TARGET_DIR}/cabify_payments-#{VERSION}.js`
  end

  task :minify => ["build:bundle"] do
    `cat #{BUILD_DIR}/minified_header.js > #{TARGET_DIR}/cabify_payments-#{VERSION}.min.js`
    `ruby #{BUILD_DIR}/jsmin.rb < #{TARGET_DIR}/cabify_payments-#{VERSION}.js >> #{TARGET_DIR}/cabify_payments-#{VERSION}.min.js`
  end
end
